...
export async function initNeonTables() {
  try {
    ...
    const sRes = await query(`SELECT COUNT(*) FROM services;`);
    if (sRes && parseInt(sRes.rows[0].count, 10) === 0) {
      await query(`
      ...
      `);
      console.log("🌱 Seeded initial services into Neon PostgreSQL.");
    }

    // Seed Expenses if empty
    const eRes = await query(`SELECT COUNT(*) FROM expenses;`);
    if (eRes && parseInt(eRes.rows[0].count, 10) === 0) {
      await query(`
      ...
      `);
      console.log("🌱 Seeded initial expenses into Neon PostgreSQL.");
    }

    console.log("✅ Neon PostgreSQL database tables & seed data initialized!");
  } catch (err) {
